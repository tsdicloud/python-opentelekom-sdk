...
class Recordset(otc_resource.OtcResource):
    ...
    _query_mapping = resource.QueryParameters(
        'name', 'zone_id', 'type', 'ttl', 'data', 'status', 'description',
        'limit', 'marker')
    ...
    zone_id = resource.URI('zone_id', default=None) # this default is needed for queries without zone_id
    #: The name of the Zone which this recordset belongs to
    zone_name = resource.Body('zone_name')

    @classmethod
    def list(cls, session, zone_id=None, **kwargs):
        """ Ectend list method to handle recordset listing without a given zone
            on the /recordsets endpoint (must be done here to also support the find case)t"""
        if not zone_id:
            base_path="/recordsets" 
            return super(Recordset, cls).list(session=session, **kwargs)
        else:
            return super(Recordset, cls).list(session=session,zone_id=zone_id,  **kwargs)

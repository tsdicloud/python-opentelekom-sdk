...
class Zone(otc_resource.OtcResource):
    """DNS ZONE Resource"""
    resources_key = 'zones'
    base_path = '/zones'
    ...
    #: OTC Zone type,
    #: Valid values for OTC include `public`, `private`
    #: *Type: str*
    zone_type = resource.Body('zone_type')
    #: OTC: Associated vpc id for private zone (OTC only)
    #: *Type: str*
    router = resource.Body('router', type=dict)
    #vpc_id = resource.Body('router_id', alias='router_id')
    #: OTC: Associated region of vpc id for private zone (OTC only)
    #: optional and usually no set 
    #: *Type: str*
